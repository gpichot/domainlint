...
const noUtilsImport: CustomRule = {
  ...
  check({ graph, config }) {
    return graph.edges
      .filter((edge) => edge.to.includes("/utils/"))
      .map((edge) => ({
...
| Property | Type | Description |
|----------|------|-------------|
| `graph` | `DependencyGraph` | The full import graph with nodes, edges, and adjacency list. |
...
The `DependencyGraph` object gives you access to:
